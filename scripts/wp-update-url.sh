...
if $(wp --path=${local_web_root} --url=${remote_domain} core is-installed --network); then
    ...
    wp search-replace --path=${local_web_root} --url=${remote_domain} ${remote_domain} ${local_domain} --recurse-objects --network --skip-columns=guid --skip-tables=wp_users --all-tables --url=%
    ...
    wp search-replace --path=${local_web_root} ${remote_domain} ${local_domain} --recurse-objects --skip-columns=guid --skip-tables=wp_users --all-tables --url=%
fi
